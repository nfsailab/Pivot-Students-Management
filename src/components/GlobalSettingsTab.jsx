...
function GlobalSettingsTab() {
  ...
  const handleAddBatch = async (e) => {
    ...
    try {
      await addDocument('batches', { batchName: newBatchName.trim() });
      setNewBatchName('');
    } catch (err) {
  ...
  const handleSaveBatch = async (id) => {
    ...
    try {
      await updateDocument('batches', id, { batchName: editBatchName.trim() });
      setEditBatchId(null);
    } catch (err) {
  ...
  const getModeIcon = (modeName) => {
    switch(modeName.toLowerCase()) {
      ...
      case 'research': return <BookOpen className="h-4 w-4 text-emerald-400" />;
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {(student.totalHours || student.totalSeconds > 0) && (
                          <span className="text-[9px] px-1.5 py-0.5 rounded bg-studio-950 border border-emerald-500/30 text-emerald-400 font-mono font-bold">
                            {student.totalHours || ((student.totalSeconds || 0) / 3600).toFixed(2)} hrs used
                          </span>
                        )}
                      </div>
                    </div>
